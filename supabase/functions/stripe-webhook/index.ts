...
Deno.serve(async (req: Request) => {
  try {
    ...
    // Webhooks don't require authentication - they use Stripe signature verification
...
async function syncCustomerFromStripe(customerId: string) {
  try {
    ...
    // Update user subscription status in users table based on subscription status
    const userStatus = subscription.status === 'active' ? 'premium' : 'free';
    console.log(`🔄 Updating user status to: ${userStatus} (subscription status: ${subscription.status})`);
    ...
    console.log(`👤 Found user_id: ${customerData.user_id} for customer: ${customerId}`);
    ...
    if (subscription.status === 'active') {
      const { error: orderError } = await supabase.from('stripe_orders').insert({
        checkout_session_id: subscription.latest_invoice, // Use invoice ID as session reference
        payment_intent_id: subscription.latest_invoice,
        customer_id: customerId,
        amount_subtotal: subscription.items.data[0].price.unit_amount,
        amount_total: subscription.items.data[0].price.unit_amount,
      ...
      if (orderError) {
        console.error('Error creating subscription order record:', orderError);
      } else {
        console.info(`Created order record for subscription: ${subscription.id}`);
      }
    }
